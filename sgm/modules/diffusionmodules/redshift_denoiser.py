...
class Denoiser(nn.Module):
    ...
    def forward(
        ...
        **additional_model_inputs,
        ...
        return (
            network(input * c_in, c_noise, cond, **additional_model_inputs) + input
        ) # network 是OpenAIWrapper下的扩散模型，input*c_skip就是对原图加噪后的结果*1

class RedshiftDenoiser(Denoiser):
    def __init__(
        self,
        scaling_config: Dict,
        num_idx: int,
        ...
        redshift = self.discretization(num_idx, do_append_zero=do_append_zero, flip=flip)
        self.register_buffer("sigmas", redshift)
