...
class GeneralVAELoss(nn.Module):
    def __init__(
        self,
        ...
        super().__init__()
        # self.perceptual_loss = LPIPS(use_random_weights=use_random_perceptual).eval()
    ...
    def forward(
        ...
        *,  # added because I changed the order here
        ...
        if self.use_mask:
            self.mask = self.generate_mask(inputs, error)
        else:
            self.mask = torch.ones_like(inputs, dtype=torch.bool)
        ...
        if optimizer_idx == 0:
            ...
            if self.ssim_weight > 0:
                loss = loss + self.ssim_weight * weighted_ssim_loss
    ...
    def mask_loss(self, loss: torch.Tensor, normalize: bool = False, inputs: torch.Tensor = None) -> torch.Tensor:
        if normalize:
    ...
    def multiscale_chi2_loss(
        ...
        for level in range(1, levels):
            ...
            if curr_mask is not None:
                ...
                temp_mask = (curr_mask_ratio > 0).float()
                # 计算损失时，考虑有效像素的比例
